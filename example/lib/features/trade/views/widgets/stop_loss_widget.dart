...
class _StopLossWidgetState extends State<StopLossWidget> {
  ...
  void initState() {
    if (widget.init != null) {
      ...
      if (widget.onChange != null && widget.init != null) {
        widget.onChange!(getTotalPrice(stopLossOptions.value, widget.init!));
      }
    }
    super.initState();
  }
  ...
  final stopLossOptions = ValueNotifier<PriceOptions>(PriceOptions.Price);
  ...
  Widget build(BuildContext context) {
    ...
    return ValueListenableBuilder(
      ...
      builder: (context, entryPrice, _) {
        totalPrice.value = getTotalPrice(stopLossOptions.value, entryPrice);
          ...
          children: [
                ...
                builder: (context, priceOption, child) {
                  return TradeTextFiledItemWidget(
                    ...
                    validator: (messageError) {
                      ...
                      if (price == 0 &&
                          stopLossOptions.value != PriceOptions.Price) {
                        return context.tr.fieldInvalid(context.tr.stopLoss);
                      } else if (price == 0) {
                        return null;
                      }
                      ...
                      if (widget.type == PositionType.ask) {
                        return entryPrice < totalPrice
                            ? context.tr.fieldInvalid(context.tr.stopLoss)
                            : null;
            ...
            Expanded(
              ...
              child: CDropDown<PriceOptions>(
                ...
                onSelection: (value) {
                  if (value != null) {
                    stopLossOptions.value = value.value;
                  }
                },
              ).addPadding(top: 27.0),
            ),
  ...
  void dispose() {
    totalPrice.dispose();
    stopLossOptions.dispose();
    super.dispose();
  }
}

//3296.50 sl => 3290  tp => 3300


//3296.50 tp => 3290  sl => 3300